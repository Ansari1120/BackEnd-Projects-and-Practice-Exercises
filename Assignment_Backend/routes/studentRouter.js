...
route.post("/", async(req,res)=>{
    let {FirstName,LastName,Contact,Courses} = req.body;
    try{
  let errArr = []
  ...
  //validation Part
      ...
      errArr.push("Required Contact");
        ...
        if(!Courses){
      errArr.push("Required Courses");
        }

    
        if(errArr.length > 0 ){
            return res.send(sendResponse(false,errArr,null,"Required All Fields")).status(400) ;
        }
        else{
            let obj = {FirstName,LastName,Contact,Courses};
